...
import generacion.ProductosMedios;
...
public class PanelProductosMedios extends JPanel {
	...
	public PanelProductosMedios() {
		...
		semillaDos = new JTextField();
		semillaDos.setForeground(Color.BLACK);
		semillaDos.addKeyListener(new KeyAdapter() {
			@Override
			public void keyTyped(KeyEvent evt) {
				char c = evt.getKeyChar();
				if (Character.isDigit(c) || evt.getKeyChar() == KeyEvent.VK_ENTER
		...
		semillaDos.setFont(new Font("Product Sans", Font.PLAIN, 16));
		semillaDos.setColumns(10);
		semillaDos.setBorder(new TitledBorder(
				new EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160, 160, 160)),
				"SEMILLA (X1)", TitledBorder.LEFT, TitledBorder.TOP, null, new Color(0, 0, 0)));
		semillaDos.setBackground(Color.WHITE);
		GridBagConstraints gbc_semillaDos = new GridBagConstraints();
		gbc_semillaDos.gridwidth = 2;
		gbc_semillaDos.insets = new Insets(0, 0, 5, 5);
		gbc_semillaDos.fill = GridBagConstraints.HORIZONTAL;
		gbc_semillaDos.gridx = 0;
		gbc_semillaDos.gridy = 2;
		panelCaptura.add(semillaDos, gbc_semillaDos);
	...
	public void calcularValores() {
		if (!areEmpty()) {
			...
			int x1 = Integer.parseInt(semillaDos.getText());
			int n = Integer.parseInt(cantidad.getText());
			ProductosMedios productosMedios = new ProductosMedios(x0,x1,n);
			productosMedios.getNumbers();
			listaX0 = productosMedios.getListaX0();
			listaY = productosMedios.getListaY();
			listaX = productosMedios.getListaX();
			listaR = productosMedios.getListaR();
